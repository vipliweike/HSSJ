    ...
    public class VisitController : ApiController
    {
        ...
        public IHttpActionResult Add()
        {
            try
            {
                ...
                var BusID = Convert.ToInt32(parameters["BusID"]);
                ...
                using (dbDataContext db = new dbDataContext())
                {
                    ...
                    if (bus != null)
                    {
                        ...
                        model.UserID = Convert.ToInt32(parameters["UserID"]);
                        model.VisitContent = parameters["VisitContent"];
                        model.IsLeader = Convert.ToInt32(parameters["IsLeader"]);
                        model.Leader = parameters["Leader"];
                        model.State = 0;
                        model.Business = bus;
                        ...
                        db.Visit.InsertOnSubmit(model);
                        db.SubmitChanges();
                        return Json(new { state = 1, msg = "添加成功", });
                    }
                    return Json(new { state = 0, msg = "商机不存在" });
                }
            }
            catch (Exception ex)
            {
        ...
        public IHttpActionResult Get(string data, string secret)
        {
            try
            {
                ...
                var BusID = Convert.ToInt32(parameters["BusID"]);//商机ID
                
                string[] imgNUll = { };//空数组
                using (dbDataContext db = new dbDataContext())
                {
                    var list = db.Visit.Where(x => x.BusID == BusID)
                        ...
                        .Select(x => new
                        {
                            ...
                            x.BusID,
                            x.UserID,
                            Img = ImgHelper.GetImgAllPath(x.Img),
                            VisitRemind = x.BusRemind.Where(t => t.State == 0).OrderByDescending(t => t.ID).Select(t => new { RemindID = t.ID, t.AddTime, }).FirstOrDefault(),//走访提醒
                        }).ToList();
