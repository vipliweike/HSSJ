<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BalanceTable.aspx.cs" Inherits="HsBusiness.BalanceManage.BalanceTable" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#ImportTable").click(function () {
                $.dialog({
                    ...
                    contentHtml: '<p>1、请先<a class="red"  href="/Interface/DownLoad.ashx?action=balance">下载模板</a>，然后填入数据。</p><p>2、选择填入数据的模板文件，点击导入。</p><p><input type="file" id="files" style="padding:0 2px;border: 1px solid #ccc;"></p>'
                });
            });
            ...
            $("#del").click(function () {
                ...
                if (window.confirm('你确定要删除吗？')) {
                    ...
                    $.ajax({
                        url: 'BalanceTable.aspx/BatchDel',
                        dataType: "json",
                        type: "POST",
            ...
            $("#delall").click(function () {
                if (window.confirm('你确定要全部删除吗(请谨慎操作！！)？')) {
                    $.ajax({
                        url: 'BalanceTable.aspx/DeleteAll',
                        dataType: "json",
                        type: "POST",
                        contentType: "application/json;charset=utf-8",
                       // data: JSON.stringify({ ids: checkedList.toString() }),
        ...
        $(function () {
            ...
            if (usertype=="区县经理"||usertype=="客户经理"||usertype=="网格助理"||usertype=="行业经理") {
                $("#ImportTable").hide();
            }
            else
            {
        ...
        function remind(smid) {
            ...
            $.ajax({
                url: 'BalanceTable.aspx/GetRemind',
                dataType: "json",
                type: "POST",
                contentType: "application/json;charset=utf-8",
        ...
        function Import() {
            ...
            $.ajax({
                url: "/Interface/ImportBalance.ashx",
                dataType: 'json',
                type: 'post',
                //contentType: "application/json;charset=utf-8",
                contentType: false,
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        result += `<tr>
							...
							<td>${data.data[i].AccessNumber || ""}</td>
                            <td>${data.data[i].AccountID || ""}</td>
                            <td>${data.data[i].UserName || ""}</td>
                            <td>${data.data[i].Responsibility || ""}</td>
                            <td>${data.data[i].Balance || ""}</td>
                            <td>${data.data[i].CustomerName || ""}</td>

                            <td>${data.data[i].NetTime || ""}</td>
                            <td>${data.data[i].WeekPrice || ""}</td>
                            <td>${data.data[i].Broadband || ""}</td>
                            ...
                            <td>
                            <button class ="btn btn-success btn-mini" onclick="window.parent.test.location.href = 'BalanceDetails.aspx?BalID=${data.data[i].ID}&state=${data.data[i].IsRead}'">查看详情</button>
        ...
        /** 暂时不用
        //跳转详情
        function toDetails(SmBaID, state) {
            if (state == 0) {
                var data = {
                    SmBaID: SmBaID,
                }
