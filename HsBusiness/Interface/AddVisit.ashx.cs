...
namespace HsBusiness.Interface
{
    ...
    /// AddVisit 的摘要说明
    /// </summary>
    public class AddVisit : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var model = JsonConvert.DeserializeObject<Visit>(data);
            
            if (!string.IsNullOrEmpty(Common.TCContext.Current.OnlineUserID))
            {
                using (dbDataContext db = new dbDataContext())
                {
                    var bus = db.Business.Where(x=>x.ID==model.BusID).FirstOrDefault();
                    if (bus != null)
                    {
                        var vis = new Visit();
                        vis.BusID = model.BusID;
                        vis.IsAgain = model.IsAgain = 0;
                        vis.IsLeader = model.IsLeader;
                        vis.IsNeed = model.IsNeed = 0;
                        vis.Leader = model.Leader;
                        vis.NextTime = model.NextTime;
                        vis.State = 0;
                        vis.VisitContent = model.VisitContent;
                        vis.VisitTime = model.VisitTime;
                        vis.AddTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                        vis.UserID = bus.UserID;
                        ...
                        db.Visit.InsertOnSubmit(vis);
                        db.SubmitChanges();
                        context.Response.Write(JsonConvert.SerializeObject(new {  state = 1, msg = "添加成功" }));
                    }
                    else
                    {
                        context.Response.Write(JsonConvert.SerializeObject(new { state = 0, msg = "添加失败，商机不存在" }));
                    }
                }
