...
{
    /// <summary>
    /// ImportTopicsProject 的摘要说明
    /// </summary>
    public class ImportTopicsProject : IHttpHandler
    {
                            ...
                            if (sheet != null)
                            {
                                using (dbDataContext db = new dbDataContext())
                                {
                                    ...
                                    for (int i = 1; i <= rowCount; i++)
                                    {
                                        ...
                                        if (row.GetCell(2) != null ? !string.IsNullOrEmpty(row.GetCell(2).ToString()) : false)
                                        {
                                            var fzr = db.Users.Where(x => x.Name == row.GetCell(2).ToString()).FirstOrDefault();//负责人
                                            if (fzr == null)
                                            {
                                            ...
                                            else
                                            {
                                                ...
                                                model.Introduce = row.GetCell(1) == null ? string.Empty : row.GetCell(1).ToString(); //领导批示
                                                model.UserID = fzr.ID;
                                                model.State = 1;//已派单
                                                model.IsRead = 0;
                                                model.Type = 1;//专题项目
